...
# [1849] 将字符串拆分为递减的连续值
#
# https://leetcode-cn.com/problems/splitting-a-string-into-descending-consecutive-values/description/
#
# algorithms
# Medium (32.89%)
# Likes:    17
...
class Solution:
    def splitString(self, s: str) -> bool:
        ...
        def helper(prev: int, prev_end: int):
            for i in range(prev_end + 1, len_s):
                curr = int(s[prev_end + 1:i + 1])
                if curr - prev != -1:
                    continue
                if i == len_s - 1:
                    return True
