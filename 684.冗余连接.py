#
# @lc app=leetcode.cn id=684 lang=python3
#
# [684] 冗余连接
#
# https://leetcode-cn.com/problems/redundant-connection/description/
#
# algorithms
# Medium (66.48%)
# Likes:    356
# Dislikes: 0
# Total Accepted:    52.5K
# Total Submissions: 79K
# Testcase Example:  '[[1,2],[1,3],[2,3]]'
#
# 在本问题中, 树指的是一个连通且无环的无向图。
# 
# 输入一个图，该图由一个有着N个节点 (节点值不重复1, 2, ..., N)
# 的树及一条附加的边构成。附加的边的两个顶点包含在1到N中间，这条附加的边不属于树中已存在的边。
# 
# 结果图是一个以边组成的二维数组。每一个边的元素是一对[u, v] ，满足 u < v，表示连接顶点u 和v的无向图的边。
# 
# 返回一条可以删去的边，使得结果图是一个有着N个节点的树。如果有多个答案，则返回二维数组中最后出现的边。答案边 [u, v] 应满足相同的格式 u <
# v。
# 
# 示例 1：
# 
# 输入: [[1,2], [1,3], [2,3]]
# 输出: [2,3]
# 解释: 给定的无向图为:
# ⁠ 1
# ⁠/ \
# 2 - 3
# 
# 
# 示例 2：
# 
# 输入: [[1,2], [2,3], [3,4], [1,4], [1,5]]
# 输出: [1,4]
# 解释: 给定的无向图为:
# 5 - 1 - 2
# ⁠   |   |
# ⁠   4 - 3
# 
# 
# 注意:
# 
# 
# 输入的二维数组大小在 3 到 1000。
# 二维数组中的整数在1到N之间，其中N是输入数组的大小。
# 
# 
# 更新(2017-09-26):
# 我们已经重新检查了问题描述及测试用例，明确图是无向 图。对于有向图详见冗余连接II。对于造成任何不便，我们深感歉意。
# 
#

# @lc code=start
class Node:
    def __init__(self, value: int) -> None:
        self.value = value
        self.neighbors: set[int] = set()


class Graph:
    def __init__(self, edges: list[list[int]]) -> None:
        self.node_map: dict[int, Node] = {}
        for n1, n2 in edges:
            self.get_node(n1).neighbors.add(n2)
            self.get_node(n2).neighbors.add(n1)

    def get_node(self, value: int) -> Node:
        if not value in self.node_map:
            self.node_map[value] = Node(value)
        return self.node_map[value]

    def get_entries(self) -> list[int]:
        ret: list[int] = []
        for n in self.node_map:
            node = self.node_map[n]
            if len(node.neighbors) <= 1:
                ret.append(node.value)
        return ret


class Solution:
    def findRedundantConnection(self, edges: list[list[int]]) -> list[int]:
        index_of_edges: dict[tuple[int, int], int] = {}
        for i, edge in enumerate(edges):
            n1, n2 = edge
            t = (n1, n2) if n1 < n2 else (n2, n1)
            index_of_edges[t] = i

        graph = Graph(edges)
        queue = graph.get_entries()
        while queue:
            curr = queue.pop(0)
            curr_node = graph.node_map.pop(curr)

            for n in curr_node.neighbors:
                node = graph.get_node(n)
                node.neighbors.remove(curr)
                if len(node.neighbors) <= 1:
                    queue.append(n)

        ret, ret_order = None, -1
        for n1 in graph.node_map:
            for n2 in graph.node_map[n1].neighbors:
                t = (n1, n2) if n1 < n2 else (n2, n1)
                order = index_of_edges[t]
                if order >= ret_order:
                    ret_order = order
                    ret = t
        return [ret[0], ret[1]]

# @lc code=end
