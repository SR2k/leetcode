...
class ThroneInheritance:
    def __init__(self, kingName: str):
        king = {}
        self.name_2_node = { kingName: king }
        self.root = { kingName: king }
        self.dead = set()


    def birth(self, parentName: str, childName: str) -> None:
        child = {}
        self.name_2_node[childName] = child
        self.name_2_node[parentName][childName] = child
    ...
    def getInheritanceOrder(self) -> list[str]:
        ...
        def helper(node, name):
            if name and name not in self.dead:
                result.append(name)

            for childName in node:
                # print(name, '-->', childName)
                # print(childName)
                helper(node[childName], childName)
...
actions = ["ThroneInheritance", "birth", "birth", "birth", "birth", "birth", "birth", "getInheritanceOrder", "death", "getInheritanceOrder"]
params = [["king"], ["king", "andy"], ["king", "bob"], ["king", "catherine"], ["andy", "matthew"], ["bob", "alex"], ["bob", "asha"], [], ["bob"], []]

for i, action in enumerate(actions):
    if action == "ThroneInheritance":
        s = ThroneInheritance(params[i][0])
    elif action == "birth":
        s.birth(params[i][0], params[i][1])
    elif action == "death":
        s.death(params[i][0])
    elif action == "getInheritanceOrder":
