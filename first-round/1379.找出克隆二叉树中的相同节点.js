...
var getTargetCopy = function(original, cloned, target) {
  // 1. 没有相同
  // if (!cloned || cloned.val === target.val) return cloned
  ...
  if (cloned.val === target.val) {
    const isSame = (treeA, treeB) => {
      ...
      if (!treeA.left && !treeB.left && !treeA.right && !treeB.right && treeA.val === treeB.val) return true

      return isSame(treeA.left, treeB.left) && isSame(treeA.right, treeB.right)
    }
    if (isSame(cloned, target)) return cloned
  }
