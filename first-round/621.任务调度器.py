...
class Solution:
    ...
    #     job_count = len(tasks)
    #     if n == 0: return job_count

    #     freq: dict[str, int] = {}
    #     for task in tasks: freq[task] = (freq.get(task) or 0) + 1

    #     tasks = list(freq.keys())
    #     tasks.sort(key=lambda x: -freq[x])

    #     next_avaliable: dict[str, int] = {}
    #     time = 1
    #     done_count = 0

    #     while True:
    #         print('\n' + str(time),next_avaliable)

    #         for task in tasks:
    #             if freq[task] and ((next_avaliable.get(task) or 0) <= time):
    #                 print('do task', task)
    #                 done_count += 1
    #                 next_avaliable[task] = time + n + 1
    #                 freq[task] -= 1
    #                 break

    #         if done_count == job_count: return time
    #         time += 1
