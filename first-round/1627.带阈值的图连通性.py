...
# [1627] 带阈值的图连通性
#
# https://leetcode-cn.com/problems/graph-connectivity-with-threshold/description/
#
# algorithms
# Hard (37.97%)
# Likes:    29
...
class Solution:
    def areConnected(self, n: int, threshold: int, queries: list[list[int]]) -> list[bool]:
        ...
        for i in range(threshold + 1, n // 2 + 1):
            ...
            while i * m <= n:
                curr_root = find_root(parents, i * m)
                parents[curr_root] = root
                m += 1

        return [find_root(parents, a) == find_root(parents, b) for a, b in queries]
# @lc code=end
