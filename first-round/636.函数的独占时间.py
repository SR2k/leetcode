...
class Solution:
    def exclusiveTime(self, n: int, logs: list[str]) -> list[int]:
        ...
        for log in logs:
            id_str, type, ts_str = log.split(':')
            id, ts = int(id_str), int(ts_str)

            if type == START:
                if call_stack:
                    prev_id, prev_start = call_stack[-1]
                    ret[prev_id] += ts - prev_start
                call_stack.append((id, ts))
            else:
                curr_id, curr_start = call_stack.pop()
                ret[curr_id] += ts - curr_start + 1
                if call_stack:
                    prev_id, prev_start = call_stack.pop()
                    call_stack.append((prev_id, ts + 1))
