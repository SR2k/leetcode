...
class Solution:
    def mostVisitedPattern(self, username: list[str], timestamp: list[int], website: list[str]) -> list[str]:
        users_to_activities: dict[str, list[tuple[timestamp, website]]] = defaultdict(list)
        for i in range(len(username)):
            user, ts, url = username[i], timestamp[i], website[i]
            users_to_activities[user].append((ts, url))

        count: defaultdict[tuple[str, str, str], int] = defaultdict(int)
        for user in users_to_activities:
            if len(users_to_activities[user]) < 3:
                continue
            sites = [x[1] for x in sorted(users_to_activities[user], key=lambda x: x[0])]
            c = set(combinations(sites, 3))
            for x in c:
                count[x] += 1
