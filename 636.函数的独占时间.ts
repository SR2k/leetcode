...
function exclusiveTime(n: number, logs: string[]): number[] {
  const list: Array<[string, number, ActionTypes]> = logs.map((log) => {
    const [id, action, ts] = log.split(':')
    return [id, +ts, action === 'start' ? ActionTypes.Push : ActionTypes.Pop]
  })
  const heap = new Heap(list, (a, b) => {
    const [__, tsA, actionA] = a
    const [_, tsB, actionB] = b

    if (tsA !== tsB) {
      return tsA - tsB
    }
    const aA: number = actionA === ActionTypes.Push ? 0 : 1
    const aB: number = actionB === ActionTypes.Push ? 0 : 1
    return aA - aB
  })

  type TaskBegin = [string, number]

  const stack: Array<string> = []
  const resultLen: Record<string, number> = {}

  let running: TaskBegin = ['$', Number.MIN_SAFE_INTEGER]

  while (heap.length) {
    const [id, ts, action] = heap.pop()!

    if (action === ActionTypes.Push) {
      stack.push(running[0])
      resultLen[running[0]] = resultLen[running[0]] || 0
      resultLen[running[0]] += ts - running[1]
      running = [id, ts]
    } else {
      resultLen[id] = resultLen[id] || 0
      resultLen[id] += ts - running[1] + 1
      running = [stack.pop()!, ts + 1]
    }
  }

  const result: number[] = []
  for (let i = 0; i < n; i++) {
    result.push(resultLen[i] || 0)
  }
  return result
}
...
class Heap<T> {
  ...
  private shouldSwap(parentIndex: number, childrenIndex: number) {
    return parentIndex >= 0 && parentIndex < this.length
      && childrenIndex >= 0 && childrenIndex < this.length
      && this.compare(this.values[parentIndex], this.values[childrenIndex]) > 0
  }
