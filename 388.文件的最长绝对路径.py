...
def is_file(s: str) -> bool:
    pt_idx = s.find('.')
    return pt_idx > 0 and pt_idx < len(s) - 1
...
class Solution:
    def lengthLongestPath(self, input: str) -> int:
        ...
        for row in rows:
            tab_cnt = count_tab(row)

            while tab_cnt < len(prefix):
                prefix_len -= len(prefix.pop())

            p = row[tab_cnt:]
            if is_file(row):
                curr_ret = len(prefix) - 1 + prefix_len + len(p)
