...
class Solution:
    def minimumTime(self, n: int, relations: list[list[int]], time: list[int]) -> int:
        ...
        for pre, post in relations:
            following[pre].append(post)
            requirements[post] += 1

        queue = deque([i for i in range(1, n + 1) if requirements[i] == 0])
        result = 0
        while queue:
            learnt = queue.popleft()
            end = begin[learnt] + time[learnt - 1]
            result = max(result, end)

            for f in following[learnt]:
                begin[f] = max(begin[f], end)
                requirements[f] -= 1
