...
class Solution:
    def isBipartite(self, graph: list[list[int]]) -> bool:
        entries = set(range(len(graph)))
        first_entry = entries.pop()

        queue = deque([(first_entry, 0)])
        set_map: dict[int, int] = { first_entry: 0 }

        while queue:
            node, set_id = queue.popleft()
            neighbor_set = 1 - set_id

            for neighbor in graph[node]:
                if neighbor in set_map:
                    if set_map[neighbor] != neighbor_set:
                        return False
                else:
                    queue.append((neighbor, neighbor_set))
                    set_map[neighbor] = neighbor_set
                    entries.remove(neighbor)

            if (not queue) and entries:
                next_entry = entries.pop()
                queue.append((next_entry, 0))
                set_map[next_entry] = 0
