...
function nextPermutation(nums: number[]): void {
  ...
  let lastGreaterIndex = i
  while (lastGreaterIndex + 1 < nums.length && nums[lastGreaterIndex + 1] > nums[i - 1]) {
    lastGreaterIndex += 1
  }

  [nums[lastGreaterIndex], nums[i - 1]] = [nums[i - 1], nums[lastGreaterIndex]]
