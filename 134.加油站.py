...
class Solution:
    @staticmethod
    def _check(raw_cost: list[int], begin: int) -> int:
        g = 0
        for i in range(begin, len(raw_cost)):
            g += raw_cost[i]
            if g < 0:
                return i
    ...
    def canCompleteCircuit(self, gas: list[int], cost: list[int]) -> int:
        raw_cost = [gas[i] - cost[i] for i in range(len(gas))]
        i = 0
        while i < len(raw_cost):
            r = Solution._check(raw_cost, i)
            if r < 0:
                return i
            elif r < i:
                i += 1
