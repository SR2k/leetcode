...
var wiggleMaxLength = function(nums) {
  // dp[i][0] - 恰好以第 i 个数字为结尾的子序列中，到 nums[i] < nums[i - 1] 的最长摇摆子序列的长度
  // dp[i][1] - 恰好以第 i 个数字为结尾的子序列中，到 nums[i] > nums[i - 1] 的最长摇摆子序列的长度
  ...
  for (let i = 0; i < nums.length; i++) {
    if (i === 0) {
      ...
      dp[i][0] = nums[i] < nums[i - 1] ? Math.max(dp[i - 1][1] + 1, dp[i - 1][0]) : dp[i - 1][0]
      dp[i][1] = nums[i] > nums[i - 1] ? Math.max(dp[i - 1][0] + 1, dp[i - 1][1]) : dp[i - 1][1]
    }
