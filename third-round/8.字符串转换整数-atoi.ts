...
function myAtoi(s: string): number {
  let sign = 1; let
    result = 0
  let status: AtoiStatus = AtoiStatus.Init
  ...
  for (const char of s) {
    if (isDigit(char)) {
      status = AtoiStatus.Number
      const i = charToInt(char)

      if (sign === 1 && result >= (MAX_POSITIVE_ABSOLUTE - i) / 10) {
        return MAX_POSITIVE_ABSOLUTE
      } if (sign === -1 && result >= (MAX_NEGATIVE_ABSOLUTE - i) / 10) {
        return -MAX_NEGATIVE_ABSOLUTE
      }

      result = result * 10 + i
    } else if (char === '+' || char === '-') {
      if (status === AtoiStatus.Init) {
        status = AtoiStatus.Sign
        sign = char === '+' ? 1 : -1
      } else if (status === AtoiStatus.Number) {
        break
      } else {
        return 0
      }
    } else if (char === ' ') {
      if (status === AtoiStatus.Number) {
        break
      } else if (status === AtoiStatus.Sign) {
        return 0
      }
    } else if (status === AtoiStatus.Number) {
      break
