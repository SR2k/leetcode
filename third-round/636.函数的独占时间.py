...
class Solution:
    def exclusiveTime(self, n: int, logs: list[str]) -> list[int]:
        ...
        for log in logs:
            ...
            if event == 'end':
                begin, _ = running
                result[id] += ts - begin + 1
                running = (ts + 1, holding_stack.pop()) if holding_stack else None
            else:
                if running:
                    prev_begin, prev_id = running
                    result[prev_id] += ts - 1 - prev_begin + 1
                    holding_stack.append(prev_id)
                running = (ts, id)
