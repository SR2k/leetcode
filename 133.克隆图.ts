...
// @lc code=start
function cloneGraph(entry: Node | null): Node | null {
  if (!entry) return entry

  const clonedMap = new Map<Node, Node>()
  const stack2Clone: Node[] = [entry]

  const clonedEntry = new Node(entry.val)
  clonedMap.set(entry, clonedEntry)

  while (stack2Clone.length) {
    const node = stack2Clone.pop()!
