...
class Solution:
    def search(self, nums: list[int], target: int) -> int:
        ...
        while left + 1 < right:
            ...
            if num_middle >= nums[0]:
                if target >= num_middle or target < nums[0]:
                    left = middle
                else:
                    right = middle
            else:
                if num_middle < target < nums[0]:
                    left = middle
                else:
                    right = middle
...
print(s.search([0,1,2,4,5,6,7], target = 3))
print(s.search([0,1,2,4,5,6,7], target = 4))
print(s.search([1], target = 0))
