...
class Solution:
    def champagneTower(self, poured: int, query_row: int, query_glass: int) -> float:
        ...
        for i in range(1, query_row):
            ...
            for j in range(min(i + 1, query_glass + 1)):
                dp[i].append(0)

                if j == 0:
                    prev = dp[i - 1][j]
                    dp[i][j] = max(prev / 2 - 1, 0)
                elif j == i:
                    prev = dp[i - 1][j - 1]
                    dp[i][j] = max(prev / 2 - 1, 0)
                else:
                    prev = dp[i - 1][j - 1] + dp[i - 1][j]
                    dp[i][j] = max(prev / 2 - 1, 0)

        print(dp)

        if query_glass == 0:
            return min(dp[query_row - 1][query_glass] / 2, 1)
        if query_glass == query_row:
            return min(dp[query_row - 1][query_glass - 1] / 2, 1)
        else:
            ret = dp[query_row - 1][query_glass - 1] / 2 + dp[query_row - 1][query_glass] / 2
            return min(ret, 1)
# @lc code=end

print(Solution().champagneTower(10, 3, 3))
