...
class Solution:
    def getSkyline(self, buildings: list[list[int]]) -> list[list[int]]:
        ...
        for left, right, height in buildings:
            heapq.heappush(corners, (left, -height, right))
            heapq.heappush(corners, (right, height, left))

        ret: list[list[int, int]] = []
        heap: list[tuple[int, int, int]] = []
        while corners:
            i, height, other_i = heapq.heappop(corners)

            if height > 0:
                heap.remove((-height, other_i, i))
                heapq.heapify(heap)
            else:
                heapq.heappush(heap, (height, i, other_i))

            next = [i, -heap[0][0] if heap else 0]
            if not ret or ret[-1][1] != next[1]:
                ret.append(next)
