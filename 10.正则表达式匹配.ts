...
// @lc code=start
const isSingleMatch = (char: string, patternChar: string) => {
  ...
  for (let i = 0; i <= s.length; i++) {
    ...
    for (let j = 0; j <= p.length; j++) {
      ...
      if (i === 0 && j === 0) {
        ...
        dp[i][j] = dp[i - 1][j - 1] && isSingleMatch(char, patternChar)
      } else {
        dp[i][j] = (
          dp[i][j - 2] || // 不匹配
          (dp[i - 1][j] || dp[i - 1][j - 2]) && isSingleMatch(char, p[j - 2]) // 匹配 1 个 or 多个
          //  && isSingleMatch(char, p[j - 2]) // 匹配
        )
      }
    }
  }
