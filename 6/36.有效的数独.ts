...
export
// @lc code=start
function isValidSudoku(board: string[][], base = 3): boolean {
  const m = base * base, n = base * base
  const rows = new Array<number>(m).fill(0)
  const cols = new Array<number>(m).fill(0)
  const zones = new Array<number>(m).fill(0)
  ...
  for (let i = 0; i < m; i++) {
    for (let j = 0; j < n; j++) {
      ...
      const z = Math.floor(i / base) * base + Math.floor(j / base)
