...
function compress(chars: string[]): number {
  ...
  for (let i = 0; i < chars.length; i++) {
    ...
    if (char === prevChar) {
      ...
      p = write(chars, prevChar, count, p)
      prevChar = char
      count = 1
    }
  }

  return write(chars, prevChar, count, p)
}

function write(chars: string[], char: string, count: number, p: number): number {
  if (!char) return p

  chars[p++] = char

  if (count > 1) {
    for (const x of String(count)) {
      chars[p++] = x
    }
  }
