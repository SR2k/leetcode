...
function myAtoi(s: string): number {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    if (char === ' ') {
      ...
      const [willExceed, addResult] = appendDigit(sign, whole, char.charCodeAt(0) - CHAR_CODE_ZERO)
      whole = addResult
      if (willExceed) {
        break
      }
...
const isNumeric = (s: string) => {
  ...
  return code >= CHAR_CODE_ZERO && code <= CHAR_CODE_NINE
}
...
const appendDigit = (sign: number, curr: number, digit: number): [boolean, number] => {
  const max = sign > 0
    ? MAX_POSITIVE_ABSOLUTE
    : MAX_NEGATIVE_ABSOLUTE

  const willExceed = curr >= (max - digit) / 10
  if (willExceed) {
    return [true, max]
  } else {
    return [false, curr * 10 + digit]
  }
}
// @lc code=end
