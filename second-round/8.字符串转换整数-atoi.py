...
class Solution:
    @staticmethod
    def judge(curr: int, digit: int, flag: int) -> tuple[bool, int]:
        edge = MAX_POS_ABS if flag == 1 else MAX_NEG_ABS
        if curr > (edge - digit) / 10:
            return False, edge
        return True, curr * 10 + digit
    ...
    def myAtoi(self, s: str) -> int:
        ...
        for char in s:
            if char == '+' or char == '-':
                ...
                status = NUMBER
                can_appand, result = Solution.judge(result, int(char), flag)
                if not can_appand:
                    return result * flag
            elif char == ' ':
                if status != INIT:
