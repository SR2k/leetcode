...
# @lc code=start
class Solution:
    def read_next(self, version: str, i: int) -> tuple[int, int]:
        if i >= len(version):
            return i, 0
        ...
        while j < len(version) and version[j] != '.':
            j += 1
        return j, int(version[i + 1:j])

    def compareVersion(self, version1: str, version2: str) -> int:
        i1, i2 = -1, -1

        while i1 < len(version1) or i2 < len(version2):
            i1, v1 = self.read_next(version1, i1)
            i2, v2 = self.read_next(version2, i2)
...
print(s.compareVersion(version1 = "1.1", version2 = "1.1"))
print(s.compareVersion(version1 = "1.0", version2 = "1.1"))
print(s.compareVersion(version1 = "1.2", version2 = "1.1"))
