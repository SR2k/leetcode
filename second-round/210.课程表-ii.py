...
class Solution:
    def findOrder(self, numCourses: int, prerequisites: list[list[int]]) -> list[int]:
        neighbors: defaultdict[int, list[int]] = defaultdict(list) # 1
        degrees = defaultdict(int) # 1
        for b, a in prerequisites: # len(prerequisites)
            neighbors[a].append(b) # 1
            degrees[b] += 1 # 1

        queue = deque(i for i in range(numCourses) if degrees[i] == 0) # numCourses
        result = []

        while queue:
            learning = queue.popleft()
            result.append(learning)

            for n in neighbors[learning]:
                degrees[n] -= 1
                if degrees[n] == 0:
                    queue.append(n)
